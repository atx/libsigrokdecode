...
class Decoder(srd.Decoder):
    ...
    def process_packet(self):
        if self.packet[0] == 0x01: # Signal Strength
            ...
            self.put_packet(
                    ["Configuration: Power Class = %d, Maximum Power = %d, Prop = %d,"
            ...
            devid = "%02x%02x%02x%02x" % (self.packet[4] & ~0x80, self.packet[5],
                        self.packet[6], self.packet[7])
            self.put_packet(
                    ["Identification: Version = %s, Manufacturer = %s, Device = %s" %
                        (version, mancode, devid),
            ...
            self.put_packet(["Extended Identification: %s" % edevid, "EI: %s" % edevid, "EI"])
        elif self.packet[0] in [0x18, 0x19, 0x28, 0x29, 0x38, 0x48, 0x58, 0x68,
                0x78, 0x85, 0xa4, 0xc4, 0xe2]: # Proprietary
            self.put_packet(["Proprietary", "P"])
        else: # Unknown
            self.put_packet(["Unknown", "?"])
        self.put(self.bytesi[-1], self.samplenum, self.out_ann,
                [6, ["Checksum OK", "OK"]] if calc_checksum(self.packet) == self.packet[-1]
    ...
    def add_bit(self, bit):
        ...
        if self.state == 'IDLE' and len(self.bits) >= 5 and self.bits[-5:] == [1, 1, 1, 1, 0]:
            self.state = 'DATA'
    ...
    def handle_transition(self, l, htl):
        self.deq.append(l)
        if len(self.deq) >= 2 and (self.in_tolerance(self.deq[-1] + self.deq[-2]) or \
                htl and self.in_tolerance(l * 2) and self.deq[-2] > 1.25 * self.bit_width):
            self.add_bit(1)
            self.deq.clear()
